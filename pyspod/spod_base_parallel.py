...
class SPOD_base(object):
    ...
    def __init__(self, data, params, data_handler, variables, weights=None):
        ...
        if data_handler:
            ...
            def data_handler(data, t_0, t_end, variables): #data_handler == data retriever
                if t_0 > t_end:
            ...
            #---- it is used to double check the dimensions of loaded data ------
            X = self._data_handler(self._data, t_0=0, t_end=0, variables=self._variables)
            if self._nv == 1 and (self._data.ndim != self._xdim + 2):
                X = X[...,np.newaxis] # the additional axis/dimension is used to take account the number of variables
        ...
        self._nx     = X[0,...,0].size #total number of spatial grid points
        self._dim    = X.ndim #number of dimension of X, e.g., X[1][50][100].ndim = 3
        self._shape  = X.shape
        self._xdim   = X[0,...,0].ndim
        self._xshape = X[0,...,0].shape #grid points along x and y
    ...
    def compute_blocks(self, iBlk):
        ...
        #s = time.time()
        if self._fftw:
            print('FFTW acceleration',flush=True)
    ...
    def compute_standard_spod(self, Q_hat_f, iFreq):
        ...
        if rank == 0:
            V = np.concatenate([np.array(item) for item in V],axis=0);
        else:
            None;
        #------------------------------------------
        ...
        if rank == 0:
            ...
            Psi = np.matmul(Q_hat_f, np.matmul(\
                V, np.diag(1. / np.sqrt(L+1e-12) / np.sqrt(self._n_blocks))));
        ...
        if rank == 0:
            tmp = self._eigs[iFreq,:] * 2 * self._n_blocks;
        else:
            tmp = [];
        tmp = comm.bcast(tmp,root=0);
        self._eigs_c[iFreq,:,0] = tmp / self._xi2_lower;           
        self._eigs_c[iFreq,:,1] = tmp / self._xi2_upper;
    ...
    def print_parameters(self):
        ...
        print('',flush=True)
        print('SPOD parameters',flush=True)
        print('------------------------------------',flush=True)
        print('Problem size               : ', self._pb_size, 'GB. (double)',flush=True)
        print('No. of snapshots per block : ', self._n_DFT,flush=True)
        print('Block overlap              : ', self._n_overlap,flush=True)
        print('No. of blocks              : ', self._n_blocks,flush=True)
        print('Windowing fct. (time)      : ', self._window_name,flush=True)
        print('Weighting fct. (space)     : ', self._weights_name,flush=True)
        print('Mean                       : ', self._mean_name,flush=True)
        print('Number of frequencies      : ', self._n_freq,flush=True)
        print('Time-step                  : ', self._dt,flush=True)
        print('Time snapshots             : ', self._nt,flush=True)
        print('Space dimensions           : ', self._xdim,flush=True)
        print('Number of variables        : ', self._nv,flush=True)
        print('Normalization weights      : ', self._normalize_weights,flush=True)
        print('Normalization data         : ', self._normalize_data,flush=True)
        print('Number of modes to be saved: ', self._n_modes_save,flush=True)
        print('Confidence level for eigs  : ', self._conf_level,flush=True)
        print('Results to be saved in     : ', self._save_dir,flush=True)
        print('Save FFT blocks            : ', self._savefft,flush=True)
        print('Reuse FFT blocks           : ', self._reuse_blocks,flush=True)
        if self._isrealx: print('Spectrum type             : ',
            'one-sided (real-valued signal)',flush=True)
        else            : print('Spectrum type             : ',
            'two-sided (complex-valued signal)',flush=True)
        print('------------------------------------',flush=True)
        print('',flush=True)
    ...
    def get_modes_at_freq(self, freq_idx):
        ...
        if self._modes is None:
            ...
            print('- RAM required for loading all modes ~', gb_memory_modes, 'GB',flush=True)
            print('- Available RAM memory               ~', gb_vram_avail  , 'GB',flush=True)
