...
from pyspod.spod_low_storage_pyfftw import SPOD_low_storage_pyfftw
from pyspod.spod_low_storage_fftw import SPOD_low_storage_fftw
...
def test_basic_spod_low_storage_fftw():
	# Initialize libraries for the low_storage algorithm
	spod_ls = SPOD_low_storage_fftw(p, params=params, data_handler=False, variables=['p'])
	spod_ls.fit()
	...
	print('RESULTS [low_storage_FFTW]')
	print(np.abs(modes_at_freq[5,10,0,0]))
	print(np.abs(modes_at_freq[0,0,0,0]))
	print(np.max(np.abs(modes_at_freq)))
...
if __name__ == "__main__":
	...
	test_basic_spod_low_storage_fftw()
